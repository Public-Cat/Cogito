...
- Env: `HOST=0.0.0.0` (listen on the container interface; LAN/WAN isolation comes from the `127.0.0.1:` port publish, not from HOST), plus `ALLOWED_ORIGINS`, `OLLAMA_BASE_URL`. Set real values before deploying (see `deploy/DEPLOY.md`). (The join code is auto-generated per session — no env var.)
