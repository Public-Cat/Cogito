...
# bring up the Caddy container first (see deploy/README.md section 2), then
...
- **Voting (normalized Borda + human votes)**: `collectAIRankings()` runs all AI rankings in parallel (`Promise.allSettled`, 20s timeout); each AI ranks active players from most to least suspicious. In `resolveRankings()`, each ranked position `i` earns `max(0, N-2-i)` points (normalized to N, not ranking length), so the top pick always equals a human vote weight (`max(1, N-2)`) regardless of partial parses, and secondary positions still contribute. In parallel, each active connected human casts one vote via `game:castVote` → `castHumanVote()`, worth `max(1, N-2)` points (floor 1 for the 2-player edge case). Self-votes and votes from eliminated/disconnected players are rejected. All points sum; highest score eliminated. Resolves early when all humans have voted and all AI rankings are done; otherwise waits for the full 40s timeout, then `resolveRankings()`. Tiebreaker 1: appears earliest in more individual AI rankings or human votes. Tiebreaker 2: cumulative vote-point history across all prior rounds. If all tiebreakers are exhausted, no one is eliminated that round and play continues (a 1v1 perfect tie ends the game via the sole-survivor rule since the human remains the last human standing).
...
- **Identity / abuse.** Player ids are random UUIDs; each player gets a `rejoinToken` and `game:rejoin` must present `{ playerId, token }`. CORS restricted via `ALLOWED_ORIGINS`. `lobby:start` validates model names against the cached Ollama list, caps AI players at `MAX_AI_PLAYERS` (8), and sanitizes/length-caps `topic`. Rate limits: pre-join actions (`lobby:setName`, `game:rejoin`) are keyed by real client IP (read from the proxy's `X-Forwarded-For`, rightmost hop; falls back to the socket address) so a reconnect can't reset the counter, with caps generous enough for a whole LAN party to join from one IP; in-game actions (`game:sendMessage`, `game:castVote`) are keyed by the joined player's stable id so many players behind one proxy/NAT IP don't share — and exhaust — a single bucket. Buckets are never cleared on disconnect; a periodic sweep prunes stale entries.

Operator runbook lives in `deploy/` (`README.md`, `Caddyfile`, `docker-compose.yml`, `.env.example`) — one flat set of files that doubles as both the production deployment (containerized Caddy + optional Cloudflare Tunnel container) and the local realm-security test harness; see `deploy/README.md`.
...
| Borda single-player ranking gave 0 points | Normalized Borda: score position i as max(0, N-2-i) so top pick always equals human vote weight |
| AI votes dominated by human votes when LLM only listed 1 name in ranking | Same normalized Borda fix — partial parses no longer under-weight the top pick |
| Borda ties stalled games with even AI splits | Add cumulative vote-point history as 3rd-level tiebreaker |
| Final 1-human-vs-1-AI standoff never resolved (each side targeting the other ties identically forever, so solo win never triggered) | Ties now produce no elimination; `checkWinCondition()` then fires and the human wins via sole-survivor rule |
